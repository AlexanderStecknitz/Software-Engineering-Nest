...
import { type Chips, modelName } from '../../chips/entity/index.js';
...
export class DbPopulateService implements OnApplicationBootstrap {
    ...
    readonly #chipsModel: mongoose.Model<Chips>;
    ...
    /**
     * Initialisierung durch DI mit `Model<Chips>` gemäß _Mongoose_.
     */
    constructor(
        dbService: DbService,
        @InjectModel(modelName) chipsModel: mongoose.Model<Chips>,
    ) {
    ...
    async #populateTestdaten() {
        ...
        try {
            await this.#chipsModel.collection.drop();
        } catch {
            this.#logger.info('#populateTestdaten: Keine Collection vorhanden');
        }

        const collection = await this.#chipsModel.createCollection();
        this.#logger.warn(
        ...
        const insertedDocs = await this.#chipsModel.insertMany(
            this.#testdaten,
            {
                lean: true,
            },
